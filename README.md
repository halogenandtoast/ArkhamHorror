...
> **Windows 用户：** 请在 [WSL2][wsl2] 终端中运行上述命令。
> Docker Desktop 与 WSL2 集成，因此 `docker compose` 命令可以在 WSL2 中正常运行。

脚本会询问您是否要下载游戏镜像（约 2.9 GB）。
如果您跳过此步骤，应用程序会自动从 CDN 加载镜像，无需额外设置。
