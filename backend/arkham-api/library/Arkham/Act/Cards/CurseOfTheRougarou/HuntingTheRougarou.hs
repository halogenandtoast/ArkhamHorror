module Arkham.Act.Cards.CurseOfTheRougarou.HuntingTheRougarou (huntingTheRougarou) where

import Arkham.Ability
import Arkham.Act.CardDefs.CurseOfTheRougarou qualified as Cards
  ...
  runMessage msg a@(HuntingTheRougarou attrs) = case msg of
    ...
    AdvanceAct aid _ _ | aid == toId attrs && onSide A attrs -> do
      ...
      if and
        ...
        then
          push
            $ chooseOne lead [Label "$standalone.curseOfTheRougarou.huntingTheRougarou.label.resolution3" [R3]]
        else
          push
            $ chooseOne
              lead
