module Arkham.Act.Cards.EdgeOfTheEarth.TheHeartOfMadness.CollapseThePylons (collapseThePylons) where
...
instance RunMessage CollapseThePylons where
  runMessage msg a@(CollapseThePylons attrs) = runQueueT $ case msg of
    ...
    DoStep 2 (AdvanceAct (isSide B attrs -> True) _ _) -> do
      ...
      selectEach (enemyIs Enemies.theNamelessMadness <> EnemyWithPlacement Unplaced) \e -> enemyMoveTo attrs e firstRamp
      pure a
