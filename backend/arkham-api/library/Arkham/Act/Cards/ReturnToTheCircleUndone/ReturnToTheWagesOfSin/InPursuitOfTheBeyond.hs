module Arkham.Act.Cards.ReturnToTheCircleUndone.ReturnToTheWagesOfSin.InPursuitOfTheBeyond (inPursuitOfTheBeyond) where
...
instance HasAbilities InPursuitOfTheBeyond where
  getAbilities = actAbilities \a ->
    ...
    , onlyOnce
        $ restricted a 3 (ExtendedCardCount (atLeast 3) CardIsBeneathActDeck)
        $ Objective
        $ forced AnyWindow
    ]
