module Arkham.Act.Cards.TheCircleUndone.InTheClutchesOfChaos.DarkKnowledgeV1 (darkKnowledgeV1) where
  ...
  getAbilities (DarkKnowledgeV1 x) =
    extend
      ...
      , onlyOnce
          $ mkAbility x 2
          $ Objective
          $ ForcedAbilityWithCost AnyWindow (GroupClueCost (PerPlayer 3) Anywhere)
      ]
