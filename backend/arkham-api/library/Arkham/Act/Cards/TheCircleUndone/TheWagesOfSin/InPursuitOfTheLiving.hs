module Arkham.Act.Cards.TheCircleUndone.TheWagesOfSin.InPursuitOfTheLiving (inPursuitOfTheLiving) where
    ...
    guard (onSide A a)
      *> [ restricted
         ...
         , restricted
             a
             2
             (ExtendedCardCount (atLeast 4) $ VictoryDisplayCardMatch $ basic "Unfinished Business")
             $ Objective
             $ forced AnyWindow
         ]
