module Arkham.Act.Cards.TheFeastOfHemlockVale.TheLostSister.FaceToCarapace (faceToCarapace) where
...
instance RunMessage FaceToCarapace where
  runMessage msg a@(FaceToCarapace attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      enemies <-
        select
          $ EnemyAt (ConnectedFrom NotForMovement (locationWithInvestigator iid) <> LocationWithTrait Dark)
