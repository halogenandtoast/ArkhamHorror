module Arkham.Act.Cards.TheForgottenAge.ShatteredAeons.WorldsBeyond (worldsBeyond) where

import Arkham.Ability
import Arkham.Act.CardDefs.TheForgottenAge.ShatteredAeons qualified as Cards
...
instance HasAbilities WorldsBeyond where
  getAbilities (WorldsBeyond a) =
    extend1 a
      $ mkAbility a 1
      $ forced
      $ EnemyAttemptsToSpawnAt #when (EnemyWantsToSpawnIn EmptyLocation) LocationNotInPlay
