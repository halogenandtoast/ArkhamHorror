...
getMove [] = Nothing
getMove ((Window.windowType -> Window.Moves _ _ (Just from) to _) : _) = Just (from, to)
...
instance RunMessage WalkingThroughTime where
  runMessage msg a@(WalkingThroughTime attrs) = runQueueT $ case msg of
    UseCardAbility iid (isSource attrs -> True) 1 (getMove -> Just (from, to)) _ -> do
      scientists <- select $ AssetWithTrait Scientist <> AssetAt (LocationWithId from)
      movable <- filterM (`assetCanEnter` to) scientists
      when (notNull movable) do
        chooseSomeM iid "Done moving scientists" do
          targets movable \scientist -> place scientist (AtLocation to)
      pure a
