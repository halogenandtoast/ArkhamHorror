module Arkham.Agenda.Cards.EdgeOfTheEarth.ToTheForbiddenPeaks.ForbiddenPeaks (forbiddenPeaks) where

import Arkham.Ability
import Arkham.Agenda.CardDefs.EdgeOfTheEarth.ToTheForbiddenPeaks qualified as Cards
    ...
    AdvanceAgenda (isSide B attrs -> True) -> do
      selectForMaybeM (HighestRow $ LocationWithInvestigator Anyone) \loc -> do
        ...
        for_ (nonEmpty partners) \ps -> do
          ...
          pushWhen isCookie $ Msg.DealDamage (EnemyTarget terror) (nonAttack Nothing (toSource attrs) 2)
