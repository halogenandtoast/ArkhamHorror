module Arkham.Agenda.Cards.EnthrallingEncore.TheHauntingOfTheWardTheatre (theHauntingOfTheWardTheatre) where
...
instance HasAbilities TheHauntingOfTheWardTheatre where
  getAbilities (TheHauntingOfTheWardTheatre a) =
    [ restricted a 1 (exists $ enemyIs Enemies.sinisterSoloist <> EnemyAt YourLocation)
        $ FastAbility'
          ( Costs
              [HandDiscardCost 1 (basic $ CardWithClass c) | c <- [Guardian, Seeker, Rogue, Mystic, Survivor]]
          )
          #parley
    ]
