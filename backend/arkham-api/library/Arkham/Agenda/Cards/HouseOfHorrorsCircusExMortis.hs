module Arkham.Agenda.Cards.HouseOfHorrorsCircusExMortis (houseOfHorrorsCircusExMortis) where
...
newtype HouseOfHorrorsCircusExMortis = HouseOfHorrorsCircusExMortis AgendaAttrs
  deriving anyclass (IsAgenda, HasModifiersFor, HasAbilities)
  deriving newtype (Show, Eq, ToJSON, FromJSON, Entity)

houseOfHorrorsCircusExMortis :: AgendaCard HouseOfHorrorsCircusExMortis
houseOfHorrorsCircusExMortis =
  agenda (2, A) HouseOfHorrorsCircusExMortis Cards.houseOfHorrorsCircusExMortis (Static 5)

instance RunMessage HouseOfHorrorsCircusExMortis where
  runMessage msg a@(HouseOfHorrorsCircusExMortis attrs) = runQueueT $ case msg of
    AdvanceAgenda (isSide B attrs -> True) ->
      campaignI18n $ scope "oneNightOnly" $ scope "houseOfHorrors" do
        ...
        pure a
    _ -> HouseOfHorrorsCircusExMortis <$> liftRunMessage msg attrs
