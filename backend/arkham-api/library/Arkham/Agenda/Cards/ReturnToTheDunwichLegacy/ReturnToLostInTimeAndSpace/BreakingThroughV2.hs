module Arkham.Agenda.Cards.ReturnToTheDunwichLegacy.ReturnToLostInTimeAndSpace.BreakingThroughV2 (breakingThroughV2) where
...
instance HasAbilities BreakingThroughV2 where
  getAbilities (BreakingThroughV2 x) =
    [mkAbility x 1 $ forced $ MovedBy #after You EncounterCardSource | onSide A x]
