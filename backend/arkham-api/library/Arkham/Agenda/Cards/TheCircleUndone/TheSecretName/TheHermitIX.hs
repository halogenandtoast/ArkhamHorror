module Arkham.Agenda.Cards.TheCircleUndone.TheSecretName.TheHermitIX (theHermitIX) where
...
instance HasModifiersFor TheHermitIX where
  getModifiersFor (TheHermitIX (a `With` _)) = do
    modifySelect a NonWeaknessEnemy [HealthModifier 1]

instance HasAbilities TheHermitIX where
  getAbilities (TheHermitIX (a `With` _)) =
    [ mkAbility a 1
        $ freeReaction
        $ EnemyDefeatedIncludingOutOfPlay
          #after
          You
          ByAny
