module Arkham.Asset.Assets.Blur1 (blur1, Blur1 (..)) where
...
instance RunMessage Blur1 where
  runMessage msg a@(Blur1 attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      ...
      chooseOne
        iid
        [ Label "$label.cards.blur1.useWillpower" [toMessage $ withSkillType #willpower evade]
        , Label "$label.cards.blur1.useAgility" [toMessage evade]
        ]
      pure a
