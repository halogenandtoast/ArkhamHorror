...
instance HasAbilities DrDewiIrawanCryptozoologist where
  getAbilities (DrDewiIrawanCryptozoologist a) =
    [ controlled
        ...
        1
        ( oneOf
            [ exists (InHandOf NotForPlay You <> CardWithHollowedCopy (OwnedBy can.shuffle.deck))
            , exists (InHandOf NotForPlay You <> basic DiscardableCard)
                <> exists (HollowedCard <> OwnedBy can.shuffle.deck)
            ]
        )
...
instance RunMessage DrDewiIrawanCryptozoologist where
  runMessage msg a@(DrDewiIrawanCryptozoologist attrs) = runQueueT $ case msg of
    ...
    HandleTargetChoice iid (isAbilitySource attrs 1 -> True) (CardIdTarget cid) -> do
      card <- fetchCard cid
      revealCard card
      hollows <- select $ HollowedCard <> basic (CardWithTitle card.title)
      let withOwners = hollows & mapMaybe (\c -> (,c) <$> c.owner) & groupSort
      for_ withOwners (uncurry shuffleCardsIntoDeck)
      gainResources iid (attrs.ability 1) (length hollows)
      pure a
