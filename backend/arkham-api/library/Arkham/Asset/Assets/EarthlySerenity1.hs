module Arkham.Asset.Assets.EarthlySerenity1 (earthlySerenity1, EarthlySerenity1 (..)) where
...
instance HasAbilities EarthlySerenity1 where
  getAbilities (EarthlySerenity1 a) = [skillTestAbility $ restrictedAbility a 1 ControlsThis actionAbility]
  ...
  runMessage msg a@(EarthlySerenity1 attrs) = runQueueT $ case msg of
    ...
    DoStep n msg'@(PassedThisSkillTest iid (isAbilitySource attrs 1 -> True)) | n > 0 -> do
      ...
      when (notNull choices && hasUses attrs) do
        ...
        chooseOne
          iid
          [ Label "$label.cards.earthlySerenity1.doNotSpendAChargeToHeal" []
          , Label "$label.cards.earthlySerenity1.spendAChargeToHeal"
