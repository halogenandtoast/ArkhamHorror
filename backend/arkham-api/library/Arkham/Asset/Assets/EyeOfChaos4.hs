          ...
          let
            handleIt assetId = do
              when (token.face == #curse) do
                lids <- select $ ConnectedLocation ForMovement <> locationWithDiscoverableCluesBy iid
                stillInPlay <- selectAny $ AssetWithId assetId

                when (stillInPlay || notNull lids) do
                  chooseOrRunOneM iid do
                    when stillInPlay do
                      (cardI18n $ labeled' "eyeOfChaos4.place1ChargeOnEyeOfChaos4") do
                        addUses attrs.source assetId Charge 1
                    unless (null lids) do
                      (cardI18n $ labeled' "eyeOfChaos4.discover1CluesAtAConnectingLocation") do
