...
instance RunMessage Flamethrower5 where
  runMessage msg a@(Flamethrower5 attrs) = runQueueT $ case msg of
    ...
    Successful (Action.Fight, EnemyTarget eid) iid _ (isTarget attrs -> True) _ -> do
      ...
      chooseOneM iid do
        (cardI18n $ labeled' "flamethrower5.doStandardDamage") $ push $ EnemyDamage eid $ attack attrs 1
        (cardI18n $ labeled' "flamethrower5.assignUpTo4DamageAmongEnemiesEngagedWithYou") do
