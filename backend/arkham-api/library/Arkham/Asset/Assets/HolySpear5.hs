...
instance HasAbilities HolySpear5 where
  getAbilities (HolySpear5 attrs) =
    [ (cardI18n $ withI18nTooltip "holySpear5.action_fight_YouGet2WillpowerAndDeal1DamageForThisAttackWhen")
        $ controlled_ attrs 1 fightAction_
    , (cardI18n $ withI18nTooltip "holySpear5.actionSearchTheChaosBagFor2BlessTokensAndSealThemOnHolySpear")
        $ controlled_ attrs 2
        $ fightAction
  ...
  runMessage msg a@(HolySpear5 attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      ...
      for_ mBlessToken $ \blessToken -> do
        chooseOne
          iid
          [ Label "$label.cards.holySpear5.releaseABlessToken" [UnsealChaosToken blessToken]
          , Label "$label.cards.holySpear5.doNotReleaseABlessToken" []
          ]
      sid <- getRandom
