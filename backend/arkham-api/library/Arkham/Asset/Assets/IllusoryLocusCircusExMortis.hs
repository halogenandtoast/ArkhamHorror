module Arkham.Asset.Assets.IllusoryLocusCircusExMortis (illusoryLocusCircusExMortis) where
...
newtype IllusoryLocusCircusExMortis = IllusoryLocusCircusExMortis AssetAttrs
  deriving anyclass (IsAsset, HasModifiersFor)
  deriving newtype (Show, Eq, ToJSON, FromJSON, Entity)

illusoryLocusCircusExMortis :: AssetCard IllusoryLocusCircusExMortis
illusoryLocusCircusExMortis = asset IllusoryLocusCircusExMortis Cards.illusoryLocusCircusExMortis

instance HasAbilities IllusoryLocusCircusExMortis where
  getAbilities (IllusoryLocusCircusExMortis x) =
    [restricted x 1 OnSameLocation actionAbility]

instance RunMessage IllusoryLocusCircusExMortis where
  runMessage msg a@(IllusoryLocusCircusExMortis attrs) = runQueueT $ case msg of
    ...
    RequestedChaosTokens (isAbilitySource attrs 1 -> True) (Just _iid) tokens -> do
      ...
      pure a
    _ -> IllusoryLocusCircusExMortis <$> liftRunMessage msg attrs
