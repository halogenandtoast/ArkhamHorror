...
instance HasAbilities SolemnVow where
  getAbilities (SolemnVow attrs) =
    [ restrictedAbility attrs 1 ControlsThis (FastAbility $ exhaust attrs)
        ...
        `withCriteria` exists (affectsOthers $ colocatedWithMatch You <> OwnsAsset (be attrs))
    ]
...
instance RunMessage SolemnVow where
  runMessage msg a@(SolemnVow attrs) = case msg of
    ...
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      hasDamage <- iid <=~> InvestigatorWithAnyDamage
      hasHorror <- iid <=~> InvestigatorWithAnyHorror

      damageAssets <- select $ assetControlledBy iid <> AssetWithDamage
      horrorAssets <- select $ assetControlledBy iid <> AssetWithHorror
