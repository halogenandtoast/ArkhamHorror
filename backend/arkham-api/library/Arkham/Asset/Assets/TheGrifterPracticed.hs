...
instance HasAbilities TheGrifterPracticed where
  getAbilities (TheGrifterPracticed a) = [controlled_ a 1 $ triggered (ScenarioEvent #when (Just You) "checkGameIcons") (exhaust a)]
...
instance RunMessage TheGrifterPracticed where
  runMessage msg a@(TheGrifterPracticed attrs) = runQueueT $ case msg of
