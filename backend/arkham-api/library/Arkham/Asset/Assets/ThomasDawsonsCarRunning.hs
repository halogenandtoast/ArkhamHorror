...
instance HasAbilities ThomasDawsonsCarRunning where
  getAbilities (ThomasDawsonsCarRunning x) =
    [ (cardI18n $ withI18nTooltip "thomasDawsonsCarRunning.ifYouAreThisVehiclesDriverDrawTheTopCardOfTheEncounterDeckTh")
        $ groupLimit PerRound
        $ restrictedAbility x 1 driverCriteria actionAbility
    , (cardI18n $ withI18nTooltip "thomasDawsonsCarRunning.ifYouAreThisVehiclesDriverYouStopTheCarFlipThisVehicleOver")
    ...
    InvestigatorEliminated iid -> do
      case assetDriver attrs of
        Just driver | driver == iid -> do
          ...
          pure $ ThomasDawsonsCarRunning $ attrs & driverL .~ mpassenger
        _ -> pure a
    HandleTargetChoice _ (isSource attrs -> True) (InvestigatorTarget iid) -> do
      pure $ ThomasDawsonsCarRunning $ attrs & driverL ?~ iid
    _ -> ThomasDawsonsCarRunning <$> liftRunMessage msg attrs
