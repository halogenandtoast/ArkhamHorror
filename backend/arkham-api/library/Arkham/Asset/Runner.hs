...
instance RunMessage AssetAttrs where
  runMessage msg a@AssetAttrs {..} = case msg of
    ...
    DefeatedAddToVictory target | target `elem` placementToAttached a.placement -> do
    ...
    Do (SpendUses source target useType' n) | isTarget a target -> do
      when (n >= a.use useType') do
        ...
        case source of
          UseAbilitySource _ s i -> insertAfterMatchingOrNow [MoveWithSkillTest afterLast] \case
