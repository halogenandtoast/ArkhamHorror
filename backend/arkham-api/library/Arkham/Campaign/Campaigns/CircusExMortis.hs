module Arkham.Campaign.Campaigns.CircusExMortis (circusExMortis) where

import Arkham.Asset.Cards qualified as Assets
import Arkham.Campaign.Import.Lifted
import Arkham.Campaigns.CircusExMortis.CampaignSteps
import Arkham.Campaigns.CircusExMortis.ChaosBag
...
instance RunMessage CircusExMortis where
  runMessage msg c = runQueueT $ campaignI18n $ case msg of
    ...
    CampaignStep (InterludeStep 1 _) -> scope "theFutureAndThePast" do
      ...
      addCampaignCardToDeckChoice_ Assets.amaltheaWeaverCircusFortuneTellerCircusExMortis
      ...
      addCampaignCardToDeckChoice_ Assets.deCultusBestiaeForgottenWorkOfApuleiusCircusExMortis
      ...
      eachInvestigator \iid -> do
        chooseOneM iid do
          ...
          labeled' "addInvocationOfDiana"
            $ addCampaignCardToDeck iid DoNotShuffleIn Skills.invocationOfDianaCircusExMortis
    ...
    CampaignStep (InterludeStep 3 _) -> scope "goodOmens" do
      ...
      case snd <$> mAmalthea of
        Just v
          | v == Assets.amaltheaWeaverAspirantOfCourageCircusExMortis ->
              storyWithChooseOneM' (setTitle "title" >> p "moreToDo") do
                labeled' "priorWarning" do
                  flavor $ setTitle "title" >> p "priorWarning"
                  swapCampaignCard v Assets.amaltheaWeaverOracleOfPurityCircusExMortis
                labeled' "sawItComing" do
                  flavor $ setTitle "title" >> p "sawItComing"
                  swapCampaignCard v Assets.amaltheaWeaverOracleOfResolveCircusExMortis
        Just v
          | v == Assets.amaltheaWeaverAspirantOfWisdomCircusExMortis ->
              storyWithChooseOneM' (setTitle "title" >> p "moreToSee") do
                labeled' "writtenInInk" do
                  flavor $ setTitle "title" >> p "writtenInInk"
                  swapCampaignCard v Assets.amaltheaWeaverOracleOfEnlightenmentCircusExMortis
                labeled' "writtenInSmoke" do
                  flavor $ setTitle "title" >> p "writtenInSmoke"
                  swapCampaignCard v Assets.amaltheaWeaverOracleOfMysteryCircusExMortis
        _ -> pure ()
      scope "theLastWord" $ flavor $ setTitle "title" >> p "body"
      mDeCultus <- getDeCultusBestiaeOwner
      case snd <$> mDeCultus of
        Just v
          | v == Assets.deCultusBestiaeInterpretationOfConvictionCircusExMortis ->
              storyWithChooseOneM' (setTitle "title" >> p "theInfinite") do
                labeled' "powersAbove" do
                  flavor $ setTitle "title" >> p "powersAbove"
                  swapCampaignCard v Assets.deCultusBestiaeProphecyOfTheBeyondCircusExMortis
                labeled' "powersBelow" do
                  flavor $ setTitle "title" >> p "powersBelow"
                  swapCampaignCard v Assets.deCultusBestiaeProphecyOfTheEternalCircusExMortis
        Just v
          | v == Assets.deCultusBestiaeInterpretationOfObsessionCircusExMortis ->
              storyWithChooseOneM' (setTitle "title" >> p "theEndless") do
                labeled' "againstTheFlood" do
                  flavor $ setTitle "title" >> p "againstTheFlood"
                  swapCampaignCard v Assets.deCultusBestiaeProphecyOfTheHordeCircusExMortis
                labeled' "againstTheStorm" do
                  flavor $ setTitle "title" >> p "againstTheStorm"
                  swapCampaignCard v Assets.deCultusBestiaeProphecyOfTheBehemothCircusExMortis
        _ -> pure ()
      flavor do
        setTitle "title"
        p "breakOfDawn"
