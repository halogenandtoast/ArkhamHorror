module Arkham.Campaign.Campaigns.DarkMatter (darkMatter) where

import Arkham.Asset.Cards qualified as Assets
import Arkham.Campaign.Import.Lifted
import Arkham.Campaigns.DarkMatter.CampaignSteps
import Arkham.Campaigns.DarkMatter.Import
  ...
  runMessage msg c = runQueueT $ campaignI18n $ case msg of
    ...
    DoStep 1 (CampaignStep (InterludeStep 1 _)) -> scope "heirToCarcosa" do
      ...
      for_ (mapToList storyCards) \(iid, cards) ->
        when (any ((== Assets.heirToCarcosaDarkMatter) . toCardDef) cards) do
          chooseOneM iid do
