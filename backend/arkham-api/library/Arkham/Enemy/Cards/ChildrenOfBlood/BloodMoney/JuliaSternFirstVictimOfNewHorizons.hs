...
instance HasModifiersFor JuliaSternFirstVictimOfNewHorizons where
  getModifiersFor (JuliaSternFirstVictimOfNewHorizons a) = do
    n <- getPlayerCount
...
instance HasAbilities JuliaSternFirstVictimOfNewHorizons where
  getAbilities (JuliaSternFirstVictimOfNewHorizons a) =
...
instance RunMessage JuliaSternFirstVictimOfNewHorizons where
  runMessage msg e@(JuliaSternFirstVictimOfNewHorizons attrs) = runQueueT $ case msg of
    ...
    DoStep 1 (UseThisAbility _ (isSource attrs -> True) 1) -> do
      ...
      pure e
    _ -> JuliaSternFirstVictimOfNewHorizons <$> liftRunMessage msg attrs
