module Arkham.Enemy.Cards.EdgeOfTheEarth.FatalMirage.MemoryOfAnUnspeakableEvil (memoryOfAnUnspeakableEvil) where
...
instance RunMessage MemoryOfAnUnspeakableEvil where
  runMessage msg e@(MemoryOfAnUnspeakableEvil attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      nonAttackEnemyDamage (Just iid) (attrs.ability 1) 3 attrs.id
      pure e
