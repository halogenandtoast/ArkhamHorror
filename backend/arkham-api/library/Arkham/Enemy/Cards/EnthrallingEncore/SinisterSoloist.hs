module Arkham.Enemy.Cards.EnthrallingEncore.SinisterSoloist (sinisterSoloist) where

import Arkham.Ability
import Arkham.ClassSymbol
import Arkham.Enemy.CardDefs.EnthrallingEncore qualified as Cards
  ...
  getModifiersFor (SinisterSoloist a) = do
    modifySelectMap a (InvestigatorAt $ locationWithEnemy a) \iid ->
      [ AdditionalPlayCostOf
          (basic $ not_ (CardWithClass Neutral))
          (HorrorCost (toSource a) (toTarget iid) 1)
      ]
  ...
  getAbilities (SinisterSoloist a) =
    extend1 a
      $ restricted
        a
        1
        ( exists (EnemyWithId a.id <> ReadyEnemy)
            <> exists (InvestigatorAt $ orConnected_ $ locationWithEnemy a)
        )
      $ forced
