  ...
  getAbilities (Lloigor a) =
    extend1 a
      $ restricted
        a
        1
        (exists $ at_ (orConnected NotForMovement $ locationWithEnemy a) <> HandWith AnyCards)
      $ forced
...
instance RunMessage Lloigor where
  runMessage msg e@(Lloigor attrs) = runQueueT $ case msg of
    UseThisAbility _ (isSource attrs -> True) 1 -> do
      selectEach (at_ (orConnected NotForMovement $ locationWithEnemy attrs) <> HandWith AnyCards) \iid -> do
