module Arkham.Enemy.Cards.TheFeastOfHemlockVale.TheLostSister.CrustaceanHybridInTheLight (crustaceanHybridInTheLight) where

import Arkham.Ability
import Arkham.Campaigns.TheFeastOfHemlockVale.Helpers
import Arkham.Card
import Arkham.Enemy.CardDefs.TheFeastOfHemlockVale.TheLostSister qualified as Cards
  ...
  getAbilities (CrustaceanHybridInTheLight a) =
    extend
      ...
      [ restricted a 1 (isDark a <> youExist LeadInvestigator)
          $ SilentForcedAbility
          $ oneOf [EnemyEnters #when Anywhere (be a), EnemySpawns #when (PlacementAt Anywhere) (be a)]
      , mkAbility a 2
          $ forced
