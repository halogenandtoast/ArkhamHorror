module Arkham.Enemy.Cards.TheInnsmouthConspiracy.AgentsOfHydra.Lloigor (lloigor) where

import Arkham.Ability
import Arkham.Enemy.CardDefs.TheInnsmouthConspiracy.AgentsOfHydra qualified as Cards
...
instance HasModifiersFor Lloigor where
  getModifiersFor (Lloigor a) = do
    valid <- selectAny $ at_ (locationWithEnemy a.id) <> HandWith (LengthIs $ EqualTo $ Static 0)
    modifySelfWhen a valid [RemoveKeyword Aloof]

instance HasAbilities Lloigor where
  getAbilities (Lloigor a) =
    extend1 a
      $ restricted a 1 (exists $ at_ (orConnected_ $ locationWithEnemy a) <> HandWith AnyCards)
      $ forced
...
instance RunMessage Lloigor where
  runMessage msg e@(Lloigor attrs) = runQueueT $ case msg of
    UseThisAbility _ (isSource attrs -> True) 1 -> do
      selectEach (at_ (orConnected_ $ locationWithEnemy attrs) <> HandWith AnyCards) \iid -> do
