module Arkham.Enemy.Cards.TheInnsmouthConspiracy.CreaturesOfTheDeep.DeepOneBull (deepOneBull, DeepOneBull (..)) where

import Arkham.Ability
import Arkham.Enemy.CardDefs.TheInnsmouthConspiracy.CreaturesOfTheDeep qualified as Cards
...
instance HasAbilities DeepOneBull where
  getAbilities (DeepOneBull a) =
    extend
      ...
      , forcedAbility a 2
          $ IfEnemyDefeated #after investigator ByAny (not_ (be a) <> EnemyWithTrait DeepOne)
      ]
   where
