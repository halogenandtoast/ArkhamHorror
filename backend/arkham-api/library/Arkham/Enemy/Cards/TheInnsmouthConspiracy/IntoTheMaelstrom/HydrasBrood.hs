module Arkham.Enemy.Cards.TheInnsmouthConspiracy.IntoTheMaelstrom.HydrasBrood (hydrasBrood) where
  ...
  getAbilities (HydrasBrood a) =
    extend1 a
      $ restricted
        ...
        ( exists
            $ IncludeOmnipotent
            $ mapOneOf enemyIs [Cards.hydraDeepInSlumber, Cards.hydraAwakenedAndEnraged]
        )
      $ forced
