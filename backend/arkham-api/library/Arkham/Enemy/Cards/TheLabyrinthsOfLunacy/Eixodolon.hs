module Arkham.Enemy.Cards.TheLabyrinthsOfLunacy.Eixodolon (eixodolon) where
  ...
  getAbilities (Eixodolon a) =
    extend1 a
      $ restricted
        a
        1
        (thisExists a ExhaustedEnemy <> exists (VictoryDisplayCardMatch harvestedPainInVictory))
      $ forced
