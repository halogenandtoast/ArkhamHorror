module Arkham.Event.Events.CrypticWritings2 (crypticWritings2) where
...
instance HasAbilities CrypticWritings2 where
  getAbilities (CrypticWritings2 x) =
    [ restricted x 1 (InYourHand <> DuringTurn (can.gain.resources You))
        $ freeReaction
...
instance RunMessage CrypticWritings2 where
  runMessage msg e@(CrypticWritings2 attrs) = runQueueT $ case msg of
    PlayThisEvent iid (is attrs -> True) -> do
      ...
      gainResources iid attrs $ if hasTenOrMoreCards then 4 else 3
      pure e
