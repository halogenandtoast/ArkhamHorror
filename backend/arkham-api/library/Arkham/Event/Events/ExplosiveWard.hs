module Arkham.Event.Events.ExplosiveWard (explosiveWard, explosiveWardEffect, ExplosiveWard (..)) where
...
instance RunMessage ExplosiveWard where
  runMessage msg e@(ExplosiveWard attrs) = runQueueT $ case msg of
    PlayThisEvent iid (is attrs -> True) -> do
      ...
      chooseOrRunOne
        iid
        [targetLabel enemy [DealDamage (EnemyTarget enemy) $ nonAttack (Just iid) (attrs.ability 1) n] | enemy <- enemies]
      pure e
