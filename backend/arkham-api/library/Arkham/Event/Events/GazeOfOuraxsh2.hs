module Arkham.Event.Events.GazeOfOuraxsh2 (gazeOfOuraxsh2, GazeOfOuraxsh2 (..)) where
      ...
      when (notNull enemies) do
        pushAll
          $ replicate
            ...
            ( chooseOne
                player
                [targetLabel enemy [DealDamage (EnemyTarget enemy) $ delayDamage $ nonAttack (Just iid) attrs 1] | enemy <- enemies]
            )
