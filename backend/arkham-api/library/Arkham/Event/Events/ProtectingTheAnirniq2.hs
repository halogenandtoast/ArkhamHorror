module Arkham.Event.Events.ProtectingTheAnirniq2 (
      ...
      case getFirst fc of
        ...
        Just c -> for_ c.owner \owner -> do
          ...
          when (canDraw || canReturn) do
            chooseOne iid
              $ [Label "$cards.label.protectingTheAnirniq.return" [AddToHand owner [c]] | canReturn]
              <> [Label "$cards.label.protectingTheAnirniq.draw" [DrawCards owner (newCardDraw attrs iid 3)] | canDraw]
      pure e
