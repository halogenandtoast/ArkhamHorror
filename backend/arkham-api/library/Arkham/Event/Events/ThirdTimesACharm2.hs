module Arkham.Event.Events.ThirdTimesACharm2 (thirdTimesACharm2, thirdTimesACharm2Effect) where
...
instance HasAbilities ThirdTimesACharm2Effect where
  getAbilities (ThirdTimesACharm2Effect a) = case a.meta of
    Just (EffectInt n) -> case a.target of
      SkillTestTarget sid ->
        [ cardI18n
            $ withI18nTooltip "thirdTimesACharm.cancel"
    ...
    UseCardAbility iid (isProxySource attrs -> True) 1 (revealedChaosTokens -> [token]) _ -> do
      case attrs.meta of
        Just (EffectInt n) -> do
          ...
          pushAll
            [ CancelEachNext Nothing attrs.source [CheckWindowMessage, DrawChaosTokenMessage, RevealChaosTokenMessage]
