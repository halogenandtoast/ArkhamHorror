...
instance RunMessage TrueAwakening2 where
  runMessage msg e@(TrueAwakening2 attrs) = runQueueT $ case msg of
    PlayThisEvent iid (is attrs -> True) -> do
      chooseOneM iid do
        whenM (can.draw.cards iid) $ withI18n $ countVar 2 $ labeled "drawCards" do
          drawCards iid attrs 2
          doStep 1 msg
        whenM (canDiscoverCluesAtYourLocation NotInvestigate iid) do
          withI18n $ countVar 1 $ labeled "discoverAtYourLocation" do
            discoverAtYourLocation NotInvestigate iid attrs 1
            doStep 2 msg
        whenM (canHaveDamageHealed attrs iid) $ withI18n $ countVar 1 $ labeled "healDamage" do
          healDamage iid attrs 1
          doStep 3 msg
        whenM (canHaveHorrorHealed attrs iid) $ withI18n $ countVar 1 $ labeled "healHorror" do
    ...
    DoStep n (PlayThisEvent iid (is attrs -> True)) -> do
      chooseOneM iid do
        when (n /= 1) do
          whenM (can.draw.cards iid) $ withI18n $ countVar 2 $ labeled "drawCards" $ drawCards iid attrs 2
        when (n /= 2) do
          whenM (canDiscoverCluesAtYourLocation NotInvestigate iid) do
            withI18n
              $ countVar 1
        ...
        when (n /= 3) do
          whenM (canHaveDamageHealed attrs iid)
            ...
            $ healDamage iid attrs 1
        when (n /= 4) do
          whenM (canHaveHorrorHealed attrs iid)
            $ withI18n
            $ countVar 1
            $ labeled "healHorror"
            $ healHorror iid attrs 1
