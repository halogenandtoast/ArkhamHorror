module Arkham.Location.Cards.BrethrenOfAsh.Arkham.DowntownArkhamSanatorium (downtownArkhamSanatorium) where

import Arkham.Ability
import Arkham.GameValue
import Arkham.Location.CardDefs.BrethrenOfAsh.Arkham qualified as Cards (downtownArkhamSanatorium)
  ...
  runMessage msg l@(DowntownArkhamSanatorium attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      ...
      chooseOrRunOneM iid do
        targets investigators $ healHorrorOn source 1
        targets allies $ healHorrorOn source 1
      pure l
