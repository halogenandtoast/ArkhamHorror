module Arkham.Location.Cards.CarnevaleOfHorrors.Gondola (gondola) where

import Arkham.Ability
import Arkham.Classes
import Arkham.GameValue
import Arkham.Location.CardDefs.CarnevaleOfHorrors qualified as Cards
    ...
    UseCardAbility iid source 1 _ _ | isSource attrs source -> do
      ...
      push
        $ chooseOne
          player
          [ Label
              "$label.test count=i:2 skill=s:\"combat\""
              [beginSkillTest sid iid (attrs.ability 1) attrs #combat (Fixed 2)]
          , Label
              "$label.test count=i:2 skill=s:\"agility\""
