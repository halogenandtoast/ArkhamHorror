module Arkham.Location.Cards.CurseOfTheRougarou.TwistedUnderbrush (twistedUnderbrush) where

import Arkham.Ability
import Arkham.Classes
import Arkham.GameValue
import Arkham.Location.CardDefs.CurseOfTheRougarou qualified as Cards (twistedUnderbrush)
...
instance HasAbilities TwistedUnderbrush where
  getAbilities (TwistedUnderbrush attrs) =
    withBaseAbilities attrs
      $ [ restrictedAbility attrs 1 Here $ ActionAbility mempty Nothing $ ActionCost 1
