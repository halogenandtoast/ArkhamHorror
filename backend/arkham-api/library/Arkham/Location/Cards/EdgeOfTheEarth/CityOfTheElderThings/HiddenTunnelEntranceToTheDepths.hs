module Arkham.Location.Cards.EdgeOfTheEarth.CityOfTheElderThings.HiddenTunnelEntranceToTheDepths (hiddenTunnelEntranceToTheDepths) where
...
import Arkham.Key
import Arkham.Location.CardDefs.EdgeOfTheEarth.CityOfTheElderThings qualified as Cards
...
instance RunMessage HiddenTunnelEntranceToTheDepths where
  runMessage msg l@(HiddenTunnelEntranceToTheDepths attrs) = runQueueT $ case msg of
    ...
    DoStep n (UseThisAbility iid (isSource attrs -> True) 1) -> do
      ...
      pure $ HiddenTunnelEntranceToTheDepths $ attrs & setMeta (n == 2)
