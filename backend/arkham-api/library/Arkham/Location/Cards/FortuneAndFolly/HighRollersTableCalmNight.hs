module Arkham.Location.Cards.FortuneAndFolly.HighRollersTableCalmNight (highRollersTableCalmNight) where

import Arkham.Ability
import Arkham.Location.CardDefs.FortuneAndFolly qualified as Cards
...
instance RunMessage HighRollersTableCalmNight where
  runMessage msg l@(HighRollersTableCalmNight attrs) = runQueueT $ case msg of
    ...
    DiscardedCards iid _ (isTarget attrs -> True) cards -> do
      ...
      whenM (orM [sequential cards, sameRank 4 cards, allSameSuit cards]) $ winGame iid attrs 8
