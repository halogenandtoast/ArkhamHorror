module Arkham.Location.Cards.GuardiansOfTheAbyss.SandsOfEgypt.UntouchedVault (untouchedVault) where
  ...
  getAbilities (UntouchedVault a) =
    extendRevealed1 a
      ...
      $ restricted
        a
        1
        (Here <> HasHistory RoundHistory Anyone (DefeatedEnemyWithTraitAt Monster a.id))
        actionAbility
