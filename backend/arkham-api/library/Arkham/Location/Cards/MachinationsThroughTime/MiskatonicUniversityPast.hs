module Arkham.Location.Cards.MachinationsThroughTime.MiskatonicUniversityPast (miskatonicUniversityPast) where
...
instance HasAbilities MiskatonicUniversityPast where
  getAbilities (MiskatonicUniversityPast a) =
    extendRevealed1 a
      $ restricted a 1 Here
      $ doubleActionAbilityWithCost (SpendTokenCost Token.Seed (TargetIs $ toTarget a))
