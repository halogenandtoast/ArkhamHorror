module Arkham.Location.Cards.MoonlitForestGlassyLakeCircusExMortis (
...
newtype MoonlitForestGlassyLakeCircusExMortis = MoonlitForestGlassyLakeCircusExMortis LocationAttrs
  deriving anyclass (IsLocation, HasModifiersFor)
  deriving newtype (Show, Eq, ToJSON, FromJSON, Entity)

moonlitForestGlassyLakeCircusExMortis :: LocationCard MoonlitForestGlassyLakeCircusExMortis
moonlitForestGlassyLakeCircusExMortis =
  locationWith
    MoonlitForestGlassyLakeCircusExMortis
    Cards.moonlitForestGlassyLakeCircusExMortis
    1
    (Static 1)
    connectsToAdjacent

instance HasAbilities MoonlitForestGlassyLakeCircusExMortis where
  getAbilities (MoonlitForestGlassyLakeCircusExMortis a) =
    extendRevealed
      a
...
instance RunMessage MoonlitForestGlassyLakeCircusExMortis where
  runMessage msg l@(MoonlitForestGlassyLakeCircusExMortis attrs) = runQueueT $ case msg of
    ...
    UseThisAbility iid (isSource attrs -> True) 2 -> do
      ...
      pure l
    _ -> MoonlitForestGlassyLakeCircusExMortis <$> liftRunMessage msg attrs
