module Arkham.Location.Cards.ReturnToNightOfTheZealot.ReturnToTheGathering.StudyAberrantGateway (studyAberrantGateway) where

import Arkham.Ability
import Arkham.GameValue
import Arkham.Location.CardDefs.ReturnToNightOfTheZealot.ReturnToTheGathering qualified as Cards (
...
instance HasAbilities StudyAberrantGateway where
  getAbilities (StudyAberrantGateway attrs) =
    extendRevealed
      attrs
      [ restricted attrs 1 (Here <> youExist LeadInvestigator) $ ActionAbility mempty Nothing $ ActionCost 2
