module Arkham.Location.Cards.ReturnToTheDunwichLegacy.ReturnToLostInTimeAndSpace.RealmsBeyondAllInOne (realmsBeyondAllInOne) where
...
import Arkham.Location.CardDefs.ReturnToTheDunwichLegacy.ReturnToLostInTimeAndSpace qualified as Cards
  ...
  getAbilities (RealmsBeyondAllInOne a) =
    extendRevealed
      ...
      , restricted a 2 criteria
          $ forced
          $ EnemyWouldBeMovedBy #when AnyEnemy
          $ SourceIsLocation (locationIs Cards.anotherDimension)
      ]
   where
...
instance RunMessage RealmsBeyondAllInOne where
  runMessage msg l@(RealmsBeyondAllInOne attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      ...
      pure l
    UseCardAbility _iid (isSource attrs -> True) 2 (getEnemy -> enemy) _ -> do
