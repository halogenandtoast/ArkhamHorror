...
instance RunMessage SlimyStreets where
  runMessage msg l@(SlimyStreets attrs) = runQueueT $ case msg of
    UseCardAbility iid (isSource attrs -> True) 1 (getCount -> n) _ -> do
    ...
    RequestedChaosTokens (isAbilitySource attrs 1 -> True) (Just iid) (map (.face) -> tokens) -> do
      ...
      replicateM_ triggering
        ...
        $ chooseOrRunOneM iid do
          countVar 1 $ labeled' "slimyStreets.takeDamage" $ assignDamage iid (attrs.ability 1) 1
          countVar 1 $ labeled' "slimyStreets.takeHorror" $ assignHorror iid (attrs.ability 1) 1
