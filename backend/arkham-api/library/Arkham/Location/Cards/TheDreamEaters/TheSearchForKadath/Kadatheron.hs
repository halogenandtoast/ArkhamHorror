module Arkham.Location.Cards.TheDreamEaters.TheSearchForKadath.Kadatheron (kadatheron, Kadatheron (..)) where
...
instance HasAbilities Kadatheron where
  getAbilities (Kadatheron attrs) =
    veiled
      attrs
      [restricted attrs 1 (Here <> can.draw.cards You) $ ActionAbility mempty Nothing $ ActionCost 3]
