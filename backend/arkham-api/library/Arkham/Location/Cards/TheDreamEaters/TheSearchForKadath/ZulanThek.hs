module Arkham.Location.Cards.TheDreamEaters.TheSearchForKadath.ZulanThek (zulanThek) where
...
instance HasAbilities ZulanThek where
  getAbilities (ZulanThek a) =
    veiled1 a
      $ restricted a 1 (exists (enemyIs Enemies.hordeOfNight <> IsHost) <> exists (investigatorAt a.id))
      $ forced
      $ RoundEnds #when
...
instance RunMessage ZulanThek where
  runMessage msg l@(ZulanThek attrs) = runQueueT $ case msg of
    UseThisAbility _ (isSource attrs -> True) 1 -> do
      selectOne (enemyIs Enemies.hordeOfNight <> IsHost) >>= traverse_ \hordeOfNight -> do
