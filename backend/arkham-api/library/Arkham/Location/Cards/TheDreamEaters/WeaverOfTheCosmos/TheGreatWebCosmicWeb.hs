module Arkham.Location.Cards.TheDreamEaters.WeaverOfTheCosmos.TheGreatWebCosmicWeb (theGreatWebCosmicWeb) where
  ...
  runMessage msg l@(TheGreatWebCosmicWeb attrs) = runQueueT $ case msg of
    ...
    FailedThisSkillTestBy iid (isAbilitySource attrs 1 -> True) n -> do
      ...
      chooseOrRunOneM iid do
        when canDiscard do
          withI18n $ countVar n $ labeled "discardCards" $ chooseAndDiscardCards iid (attrs.ability 1) n
        scenarioI18n
