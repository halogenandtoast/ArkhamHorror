module Arkham.Location.Cards.TheFeastOfHemlockVale.FateOfTheVale.MirrorNest_169 (mirrorNest_169) where

import Arkham.Ability
import Arkham.Helpers.Act (getCurrentAct)
import Arkham.Helpers.Message.Discard.Lifted
import Arkham.Helpers.Modifiers (ModifierType (..))
import Arkham.Location.CardDefs.TheFeastOfHemlockVale.FateOfTheVale qualified as Cards
  ...
  runMessage msg l@(MirrorNest_169 attrs) = runQueueT $ case msg of
    ...
    UseThisAbility iid (isSource attrs -> True) 2 -> do
      ...
      search
        iid
        (attrs.ability 2)
        (ScenarioDeckTarget AbyssDeck)
        [fromDeck]
        (basic notOthersSignature)
        (defer attrs IsDraw)
      pure l
