module Arkham.Location.Cards.TheFeastOfHemlockVale.HemlockHouse.WashroomHemlockHouse37 (washroomHemlockHouse37) where
...
washroomHemlockHouse37 =
  locationWith WashroomHemlockHouse37 Cards.washroomHemlockHouse37 4 (PerPlayer 1)
    $ connectsToAdjacent
    . (canBeFlippedL .~ True)
  ...
  runMessage msg l@(WashroomHemlockHouse37 attrs) = runQueueT $ case msg of
    UseThisAbility _iid (isSource attrs -> True) 1 -> do
      for_ attrs.position \(Pos x y) -> do
        ...
        for_ mBelow \below -> moveTokens (attrs.ability 1) attrs below Clue 1
      pure l
