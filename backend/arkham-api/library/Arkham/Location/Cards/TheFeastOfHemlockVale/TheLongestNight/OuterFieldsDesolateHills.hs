module Arkham.Location.Cards.TheFeastOfHemlockVale.TheLongestNight.OuterFieldsDesolateHills (outerFieldsDesolateHills) where
...
outerFieldsDesolateHills =
  symbolLabel
    $ locationWith
      (OuterFieldsDesolateHills . (`with` Meta []))
      Cards.outerFieldsDesolateHills
      2
      (PerPlayer 3)
      connectsToAdjacent
...
instance RunMessage OuterFieldsDesolateHills where
  runMessage msg (OuterFieldsDesolateHills (With attrs meta)) = runQueueT $ case msg of
    ...
    HandleTargetChoice _iid (isAbilitySource attrs 1 -> True) (CardIdTarget cid) -> do
      let (discarded, remaining) = partition ((== cid) . toCardId) (topCards meta)
      discardFromEnemyDeck discarded
      pure $ OuterFieldsDesolateHills $ attrs `with` Meta remaining
