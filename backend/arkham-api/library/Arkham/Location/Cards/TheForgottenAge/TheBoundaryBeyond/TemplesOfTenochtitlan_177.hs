module Arkham.Location.Cards.TheForgottenAge.TheBoundaryBeyond.TemplesOfTenochtitlan_177 (templesOfTenochtitlan_177) where

import Arkham.Ability
import Arkham.Discover
import Arkham.GameValue
import Arkham.Helpers.Query
import Arkham.Location.CardDefs.TheForgottenAge.TheBoundaryBeyond qualified as Cards
...
instance HasAbilities TemplesOfTenochtitlan_177 where
  getAbilities (TemplesOfTenochtitlan_177 a) =
    extendRevealed
      ...
      , groupLimit PerRound
          $ restricted a 2 (Here <> canDiscoverCluesAt (be a) <> exists AnyEnemy)
