module Arkham.Location.Cards.TheInnsmouthConspiracy.ALightInTheFog.HoldingCells (holdingCells) where

import Arkham.Ability
import Arkham.Helpers.Modifiers
import Arkham.Helpers.SkillTest (getSkillTest)
import Arkham.Key
import Arkham.Location.CardDefs.TheInnsmouthConspiracy.ALightInTheFog qualified as Cards
...
instance HasModifiersFor HoldingCells where
  getModifiersFor (HoldingCells a) = do
    modifySelf a [CannotBeEnteredBy AnyEnemy, CannotBeFlooded]
    modifySelect a AnyEnemy [ChangeSpawnLocation (be a) (LocationWithTitle "Sunken Grotto")]
