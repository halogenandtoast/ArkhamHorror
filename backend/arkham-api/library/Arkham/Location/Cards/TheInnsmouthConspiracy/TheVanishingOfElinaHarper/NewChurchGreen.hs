module Arkham.Location.Cards.TheInnsmouthConspiracy.TheVanishingOfElinaHarper.NewChurchGreen (newChurchGreen, NewChurchGreen (..)) where
...
instance RunMessage NewChurchGreen where
  runMessage msg l@(NewChurchGreen attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      ...
      for_ cards \card -> whenNothing_ card.victoryPoints (discard card)
      pure l
