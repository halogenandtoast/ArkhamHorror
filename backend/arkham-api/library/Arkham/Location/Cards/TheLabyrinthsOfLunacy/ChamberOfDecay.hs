module Arkham.Location.Cards.TheLabyrinthsOfLunacy.ChamberOfDecay (chamberOfDecay) where
  ...
  getAbilities (ChamberOfDecay a) =
    extendRevealed1 a
      $ restricted
        a
        1
        (Here <> thisExists a LocationWithoutClues <> exists (SetAsideCardMatch $ cardIs Assets.decayDiagram))
      $ FastAbility Free
