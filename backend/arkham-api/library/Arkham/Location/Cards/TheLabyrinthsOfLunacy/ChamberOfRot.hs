module Arkham.Location.Cards.TheLabyrinthsOfLunacy.ChamberOfRot (chamberOfRot) where
  ...
  getAbilities (ChamberOfRot a) =
    extendRevealed1 a
      $ restricted
        a
        1
        (Here <> thisExists a LocationWithoutClues <> exists (SetAsideCardMatch $ cardIs Assets.rotDiagram))
      $ FastAbility Free
