module Arkham.Location.Cards.ThePathToCarcosa.BlackStarsRise.ChoeurGothique_292 (choeurGothique_292) where

import Arkham.Ability
import Arkham.Agenda.Sequence (AgendaSide (A))
import Arkham.GameValue
import Arkham.Helpers.Investigator
import Arkham.Location.CardDefs.ThePathToCarcosa.BlackStarsRise qualified as Cards
...
instance RunMessage ChoeurGothique_292 where
  runMessage msg l@(ChoeurGothique_292 attrs) = runQueueT $ case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      ...
      canHeal <- canHaveDamageHealed source iid
      when canHeal $ healDamage iid source 2
      pure l
