module Arkham.Location.Cards.TheScarletKeys.BeyondTheBeyond.RamblingRouteB (ramblingRouteB) where
...
import Arkham.Location.CardDefs.TheScarletKeys.BeyondTheBeyond qualified as Cards
...
instance HasModifiersFor RamblingRouteB where
  getModifiersFor (RamblingRouteB a) = whenRevealed a do
    for_ a.position \pos ->
      modifySelect
        a
