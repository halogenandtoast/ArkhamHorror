module Arkham.Location.Cards.TheScarletKeys.CongressOfTheKeys.CoterieLibrarySanctum (coterieLibrarySanctum, coterieLibrarySanctumEffect) where
...
instance RunMessage CoterieLibrarySanctumEffect where
  runMessage msg e@(CoterieLibrarySanctumEffect attrs) = runQueueT $ case msg of
    CardEnteredPlay iid card -> do
      when (attrs.target.investigator == Just iid && cardMatch card (#asset :: CardMatcher))
        $ disable attrs
      pure e
