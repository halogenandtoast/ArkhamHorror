module Arkham.Location.Cards.WarOfTheOuterGods.StreetsOfNewYorkCity (streetsOfNewYorkCity) where

import Arkham.Ability
import Arkham.Location.CardDefs.WarOfTheOuterGods qualified as Cards
  ...
  getAbilities (StreetsOfNewYorkCity a) =
    extendRevealed1 a
      ...
      $ restricted
        a
        1
        (exists $ EnemyAt (LocationWithTrait NewYorkCity) <> not_ (EnemyWithTrait AncientOne))
      $ doubleActionAbility
