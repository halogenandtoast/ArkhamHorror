...
resignAction a = restricted a 99 Here $ ActionAbility [#resign] Nothing (ActionCost 1)

resignActionWithCost :: (HasCardCode a, Sourceable a) => a -> Cost -> Ability
resignActionWithCost a cost = restricted a 99 Here $ ActionAbility [#resign] Nothing (ActionCost 1 <> cost)
