module Arkham.Scenario.Scenarios.ReturnToTheForgottenAge.ReturnToTheUntamedWilds (returnToTheUntamedWilds) where

import Arkham.Scenario.Import.Lifted
import Arkham.Scenario.Scenarios.TheForgottenAge.TheUntamedWilds
...
returnToTheUntamedWilds difficulty =
  scenarioWith
  ...
  runMessage msg (ReturnToTheUntamedWilds theUntamedWilds'@(TheUntamedWilds attrs)) = runQueueT $ scenarioI18n $ case msg of
    Setup ->
      runScenarioSetup
        (ReturnToTheUntamedWilds . TheUntamedWilds)
        attrs
        (setIsReturnTo >> setupTheUntamedWilds attrs)
