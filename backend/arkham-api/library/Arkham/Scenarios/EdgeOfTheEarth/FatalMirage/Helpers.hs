module Arkham.Scenarios.EdgeOfTheEarth.FatalMirage.Helpers where
...
handleMemory source partner location memory = do
  getPartnerStatus partner >>= \case
    Eliminated -> do
      ...
      -- Could be wrong to not pass an iid here, but we do not currently have one
      addToVictory_ (asId source)
      removeLocation loc
      mayAdvance source
