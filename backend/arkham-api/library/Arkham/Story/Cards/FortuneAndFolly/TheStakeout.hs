module Arkham.Story.Cards.FortuneAndFolly.TheStakeout (theStakeout) where
...
import Arkham.Story.CardDefs.FortuneAndFolly qualified as Cards
  ...
  getAbilities (TheStakeout a) =
    [ mkAbility a 1
        ...
        $ InvestigatorEliminated
          #when
          (You <> oneOf [InvestigatorWithAnyClues, InvestigatorWithResources (atLeast 1)])
    ]
