module Arkham.Story.Cards.MachinationsThroughTime.ANobleLegacyPast (aNobleLegacyPast) where
...
instance HasAbilities ANobleLegacyPast where
  getAbilities (ANobleLegacyPast a) =
    ...
    , restricted a 3 (youAtPastUniversity <> not_ (Remembered FundingForAnObservatoryHasBegun))
        $ FastAbility (CalculatedResourceCost (GameValueCalculation $ PerPlayer 2))
    , onlyOnce
        $ restricted
          a
          4
