module Arkham.Story.Cards.ReturnToThePathToCarcosa.ReturnToTheLastKing.ReturnToSickeningReality_23 (returnToSickeningReality_23) where

import Arkham.Asset.Cards qualified as Assets
import Arkham.Enemy.CardDefs.ReturnToThePathToCarcosa.ReturnToTheLastKing qualified as Enemies
import Arkham.Helpers.Location (withLocationOf)
import Arkham.Matcher
import Arkham.Story.CardDefs.ReturnToThePathToCarcosa.ReturnToTheLastKing qualified as Cards
...
instance RunMessage ReturnToSickeningReality_23 where
  runMessage msg s@(ReturnToSickeningReality_23 attrs) = runQueueT $ case msg of
    ResolveThisStory _ (is attrs -> True) -> do
      withMatch (assetIs Assets.dianneDevineHidingAnOathUnspoken) \dianne -> do
