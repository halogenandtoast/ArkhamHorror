module Arkham.Story.Cards.TheDreamEaters.PointOfNoReturn.ScoutingTheVale (ScoutingTheVale (..), scoutingTheVale) where
...
instance RunMessage ScoutingTheVale where
  runMessage msg s@(ScoutingTheVale attrs) = runQueueT $ case msg of
    ResolveThisStory iid (is attrs -> True) -> do
      n <- perPlayer 2
      lookAt iid attrs EncounterDeckTarget [peekTopOfDeck n] #any (defer attrs IsNotDraw)
