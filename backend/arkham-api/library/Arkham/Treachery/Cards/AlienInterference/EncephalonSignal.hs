module Arkham.Treachery.Cards.AlienInterference.EncephalonSignal (encephalonSignal, EncephalonSignal (..)) where
...
import Arkham.Treachery.CardDefs.AlienInterference qualified as Cards
    ...
    FailedThisSkillTest iid (isSource attrs -> True) -> do
      ...
      push
        $ chooseOne
          ...
          , Label
              "$cards.label.encephalonSignal.damage"
              [ chooseOrRunOne player
                  $ [ targetLabel investigator [assignDamage investigator (toSource attrs) 2]
                    | investigator <- investigators
                    ]
                  <> [ targetLabel humanoid [DealDamage (EnemyTarget humanoid) $ nonAttack (Just iid) (toSource attrs) 2]
                     | humanoid <- humanoids
                     ]
              ]
          ]
