module Arkham.Treachery.Cards.GuardiansOfTheAbyss.SandsOfEgypt.DarkSacrifice (darkSacrifice) where
...
instance RunMessage DarkSacrifice where
  runMessage msg t@(DarkSacrifice attrs) = runQueueT $ case msg of
    Revelation iid (isSource attrs -> True) -> do
      hasCopy <-
        selectAny $ treacheryIs Cards.darkSacrifice <> TreacheryInThreatAreaOf (InvestigatorWithId iid)
      if hasCopy
        then do
          gainSurge attrs
