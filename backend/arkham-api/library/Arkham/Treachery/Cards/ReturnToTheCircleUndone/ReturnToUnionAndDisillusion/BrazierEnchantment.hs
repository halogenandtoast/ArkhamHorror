module Arkham.Treachery.Cards.ReturnToTheCircleUndone.ReturnToUnionAndDisillusion.BrazierEnchantment (brazierEnchantment) where
...
instance HasAbilities BrazierEnchantment where
  getAbilities (BrazierEnchantment a) =
    [skillTestAbility $ mkAbility a 1 $ ActionAbility #circle Nothing (ActionCost 1)]
