module Arkham.Treachery.Cards.TheBlobThatAteEverything.CausticDissemination (causticDissemination) where
...
import Arkham.Treachery.CardDefs.TheBlobThatAteEverything qualified as Cards
...
instance HasAbilities CausticDissemination where
  getAbilities (CausticDissemination a) =
    ...
    , mkAbility a 2
        $ SilentForcedAbility
        $ AssetDefeated #when (BySource $ SourceIs $ toSource a) AnyAsset
    ]
