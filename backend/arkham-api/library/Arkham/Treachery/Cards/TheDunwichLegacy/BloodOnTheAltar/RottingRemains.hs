module Arkham.Treachery.Cards.TheDunwichLegacy.BloodOnTheAltar.RottingRemains (rottingRemains) where

import Arkham.Treachery.CardDefs.TheDunwichLegacy.BloodOnTheAltar qualified as Cards
...
rottingRemains =
  treachery
    (RottingRemains . Base.RottingRemains)
    Cards.rottingRemains

instance RunMessage RottingRemains where
  runMessage msg (RottingRemains inner) =
    RottingRemains <$> runMessage msg inner
