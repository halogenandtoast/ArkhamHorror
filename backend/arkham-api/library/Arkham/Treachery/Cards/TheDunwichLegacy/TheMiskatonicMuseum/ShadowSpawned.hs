module Arkham.Treachery.Cards.TheDunwichLegacy.TheMiskatonicMuseum.ShadowSpawned (shadowSpawned) where
...
instance RunMessage ShadowSpawned where
  runMessage msg t@(ShadowSpawned attrs) = runQueueT $ case msg of
    ...
    RemovedFromPlay (EnemySource eid) | toTarget eid `elem` attrs.attached -> pure t
