module Arkham.Treachery.Cards.TheFeastOfHemlockVale.TheLongestNight.Incursion (incursion) where
      ...
      enemies <-
        select
          $ NearestEnemyToLocation
            farmhouse
            (EnemyWithTrait Mutated <> not_ (EnemyOneOf $ map EnemyWithId chosen))
