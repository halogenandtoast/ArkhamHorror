module Arkham.Treachery.Cards.TheForgottenAge.TheCityOfArchives.CaptiveMind (captiveMind) where

import Arkham.Helpers.Message.Discard.Lifted
import Arkham.Helpers.SkillTest (getTotalModifiedSkillValue)
import Arkham.Investigator.Projection ()
import Arkham.Source
import Arkham.Treachery.CardDefs.TheForgottenAge.TheCityOfArchives qualified as Cards
...
instance RunMessage CaptiveMind where
  runMessage msg t@(CaptiveMind attrs) = runQueueT $ case msg of
    ...
    PassedThisSkillTest iid (isSource attrs -> True) -> do
      ...
      keep <- getTotalModifiedSkillValue
