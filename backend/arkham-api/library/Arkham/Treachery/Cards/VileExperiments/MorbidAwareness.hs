module Arkham.Treachery.Cards.VileExperiments.MorbidAwareness (morbidAwareness, MorbidAwareness (..)) where
  ...
  runMessage msg t@(MorbidAwareness attrs) = case msg of
    ...
    FailedThisSkillTest iid (isSource attrs -> True) -> do
      ...
      push
        ...
        <> [ Label
               (withI18n $ numberVar "count" 2 $ "$" <> ikey "label.takeHorror")
               [assignHorror iid (toSource attrs) 2]
           ]
