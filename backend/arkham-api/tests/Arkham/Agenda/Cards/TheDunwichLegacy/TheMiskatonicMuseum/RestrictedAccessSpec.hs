module Arkham.Agenda.Cards.TheDunwichLegacy.TheMiskatonicMuseum.RestrictedAccessSpec (spec) where
...
in effect while resolving the b-side of an agenda. Restricted Access prints
its forced ability on side A; its b-side spawns the Hunting Horror, so the
...
spec = describe "Restricted Access" do
  context "side-A forced ability (attach Shadow Spawned when Hunting Horror enters play)" do
    it "fires while the agenda is on side A" . gameTest $ \_self -> do
      _ <- realAgendaOnSide Agendas.restrictedAccess A
    ...
    it "still fires while the agenda is on side B (during b-side resolution)" . gameTest $ \_self -> do
      _ <- realAgendaOnSide Agendas.restrictedAccess B
      location <- testLocation
      huntingHorror <- testEnemyWithDef Enemies.huntingHorror id
      huntingHorror `spawnAt` location
      useForcedAbility
