module Arkham.Asset.Assets.BrandOfCthugha4Spec (spec) where
...
spec :: Spec
spec = describe "Brand of Cthugha (4)" do
  faq "If attack misses" do
    it "and play Oops! then 0 damage will be dealt" . gameTest $ \self -> do
      ...
      self `addToHand` oops
      enemy1 <- testEnemy & prop @"health" 3 & prop @"fight" 3
      enemy2 <- testEnemy
      brandOfCthugha4 <- self `putAssetIntoPlay` Assets.brandOfCthugha4
      location <- testLocation
      enemy1 `spawnAt` location
      enemy2 `spawnAt` location
      self `moveTo` location

      [doFight] <- brandOfCthugha4.abilities
