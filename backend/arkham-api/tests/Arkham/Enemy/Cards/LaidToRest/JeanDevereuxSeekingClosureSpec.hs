module Arkham.Enemy.Cards.LaidToRest.JeanDevereuxSeekingClosureSpec (spec) where
...
spec = describe "Jean Devereux (\"Seeking Closure\")" do
  it
    "draws the chosen Heretic: removes it from game and resolves Unfinished Business into the threat area"
    $ gameTest
    $ \investigator -> do
