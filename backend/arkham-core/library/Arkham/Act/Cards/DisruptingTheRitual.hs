...
instance RunMessage DisruptingTheRitual where
  runMessage msg a@(DisruptingTheRitual attrs@ActAttrs {..}) = case msg of
    UseCardAbility iid source 1 _ _ | isSource attrs source -> a <$ push
      (chooseOne
        iid
        [ SkillLabel SkillWillpower [beginSkillTest iid source (toTarget attrs) SkillWillpower 3]
        , SkillLabel SkillAgility [beginSkillTest iid source (toTarget attrs) SkillAgility 3]
        ]
      )
