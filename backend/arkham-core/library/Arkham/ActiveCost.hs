...
startPayment
  ...
  -> Bool
  -> GameT ()
startPayment activeCost@ActiveCost { activeCostId } iid window abilityType abilitySource abilityDoesNotProvokeAttacksOfOpportunity
  = case abilityType of
    ...
    ForcedAbilityWithCost _ cost ->
      push (PayCost activeCostId abilitySource iid Nothing False cost)
    AbilityEffect cost ->
      push (PayCost activeCostId abilitySource iid Nothing False cost)
    FastAbility cost ->
      push (PayCost activeCostId abilitySource iid Nothing False cost)
    ...
    ActionAbilityWithBefore mAction _ cost -> do
      ...
      pushAll
        (PayCost activeCostId abilitySource iid mAction True cost
    ...
    ActionAbilityWithSkill mAction _ cost ->
      if mAction
        ...
        then pushAll
          (PayCost activeCostId abilitySource iid mAction False cost
          : [ TakenAction iid action | action <- maybeToList mAction ]
          <> [ CheckAttackOfOpportunity iid False
             | not abilityDoesNotProvokeAttacksOfOpportunity
             ]
    ...
    CreatedCost acId | acId == activeCostId c -> do
      ...
      case activeCostTarget c of
        ForAbility a@Ability {..} -> do
          ...
          pure c
        _ -> error "unhandled"
    PayCost acId source iid mAction skipAdditionalCosts cost
      | acId == activeCostId c -> do
        let withPayment payment = pure $ c & costPaymentsL <>~ payment
        case cost of
          Costs xs ->
            c
              <$ pushAll
                   [ PayCost acId source iid mAction skipAdditionalCosts x
                   | x <- xs
                   ]
          UpTo 0 _ -> pure c
          UpTo n cost' -> do
                ...
                let
                  paymentOptions = map
                    (\(iid', clues) ->
                      ( iid'
                      , (0, clues)
                      , PayCost acId source iid' mAction True (ClueCost 1)
                      )
                    )
                    iidsWithClues
