...
instance HasModifiersFor ChuckFergus5 where
  getModifiersFor (InvestigatorTarget iid) (ChuckFergus5 a) | controlledBy a iid && not (assetExhausted a) = pure
...
instance RunMessage ChuckFergus5 where
  runMessage msg a@(ChuckFergus5 attrs) = case msg of
    UseCardAbility iid (isSource attrs -> True) 1 ws@(getWindowCard -> card@(PlayerCard pc)) _ -> do
      ...
      canAffordCost <- getCanAffordCost iid (PlayerCardSource pc) (Just Action.Play) ws (ResourceCost cost)
      canAffordActionCost <- getCanAffordCost iid (PlayerCardSource pc) (Just Action.Play) ws (ActionCost 1)
      -- can something else reduce the cost enough?
      let n = if canAffordCost then 2 else 1
          n' = if canAffordActionCost then n else n - 1
      when (n' > 0) $ do
        push $ chooseN iid n'
          $ [ Label "That event gains fast"
          ...
          | canAffordActionCost ] <>
          [ Label "That event costs 2 fewer resources to play."
            [ CreateWindowModifierEffect
                EffectCostWindow
                (EffectModifiers
          ...
          | canAffordCost]
          <> [ Label "You get +2 skill value while performing a skill test during the resolution of that event."
               [ CreateWindowModifierEffect
                  EffectEventWindow
                  (EffectModifiers
