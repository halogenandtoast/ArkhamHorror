...
instance RunMessage Clairvoyance5 where
  runMessage msg a@(Clairvoyance5 attrs) = case msg of
    UseCardAbility iid (isSource attrs -> True) 1 _ _ -> do
      ...
      pushAll
        $ [ createCardEffect Cards.clairvoyance5 Nothing source (InvestigationTarget iid lid)
          , skillTestModifiers attrs iid [DiscoveredClues 2, SkillModifier #willpower 3]
          ]
