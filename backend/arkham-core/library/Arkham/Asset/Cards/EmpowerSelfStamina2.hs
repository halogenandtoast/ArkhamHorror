module Arkham.Asset.Cards.EmpowerSelfStamina2 (
  empowerSelfStamina2,
  EmpowerSelfStamina2 (..),
)
where
...
instance HasAbilities EmpowerSelfStamina2 where
  getAbilities (EmpowerSelfStamina2 a) =
...
instance RunMessage EmpowerSelfStamina2 where
  runMessage msg a@(EmpowerSelfStamina2 attrs) = case msg of
    UseThisAbility iid (isSource attrs -> True) 1 -> do
      push $ skillTestModifier (toAbilitySource attrs 1) iid (SkillModifier #combat 2)
      pure a
