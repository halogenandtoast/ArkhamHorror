...
instance HasModifiersFor LonnieRitter where
  getModifiersFor (InvestigatorTarget iid) (LonnieRitter a) | controlledBy a iid =
...
instance HasAbilities LonnieRitter where
  getAbilities (LonnieRitter a) =
    [ restrictedAbility a 1
        ( ControlsThis
        <> AssetExists (HealableAsset (toSource a) DamageType (AssetWithTrait Item <> AssetControlledBy (InvestigatorAt YourLocation)))
        )
      $ FastAbility
...
instance RunMessage LonnieRitter where
  runMessage msg a@(LonnieRitter attrs) = case msg of
    UseCardAbility iid (isSource attrs -> True) 1 _ _ -> do
      items <- selectList $ HealableAsset (toSource attrs) DamageType $ AssetWithTrait Item <> AssetControlledBy (InvestigatorAt $ locationWithInvestigator iid)
