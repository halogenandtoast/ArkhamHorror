...
scryingMirror =
  asset ScryingMirror Cards.scryingMirror

instance HasAbilities ScryingMirror where
  getAbilities (ScryingMirror a) =
    [ restrictedAbility a 1 ControlsThis
      $ ReactionAbility (InitiatedSkillTest Timing.When (InvestigatorAt YourLocation) AnySkillType AnySkillTestValue)
...
instance RunMessage ScryingMirror where
  runMessage msg a@(ScryingMirror attrs) = case msg of
    UseCardAbility _iid (isSource attrs -> True) 1 _ _ -> do
      push $ skillTestModifier (toSource attrs) SkillTestTarget RevealTokensBeforeCommittingCards
      pure a
