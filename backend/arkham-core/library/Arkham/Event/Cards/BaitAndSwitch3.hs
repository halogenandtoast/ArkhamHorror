...
baitAndSwitch3Matcher iid attrs = \case
  ...
  2 -> CanEvadeEnemyWithOverride $ CriteriaOverride $ EnemyCriteria $ ThisEnemy $ EnemyAt ConnectedLocation <> NonEliteEnemy
  _ -> error "Invalid choice"

instance RunMessage BaitAndSwitch3 where
  runMessage msg e@(BaitAndSwitch3 (attrs@EventAttrs {..} `With` meta)) = case msg of
    InvestigatorPlayEvent iid eid mTarget ws _ | eid == eventId -> do
      ...
      push $ chooseOrRunOne iid $
        [ Label "Evade. If you succeed and the enemy is non-Elite, evade the enemy and move it to a connecting location." [ ResolveEventChoice iid eid 1 mTarget ws] | notNull (traceShowId choice1Enemies)
        ] <> [ Label "Evade. Use only on a non-Elite enemy at a connecting location. If you succeed, evade that enemy and switch locations with it." [ ResolveEventChoice iid eid 2 mTarget ws]
    ...
    ResolveEventChoice iid eid n _ _ | eid == eventId -> do
      ...
      when (n == 2) $ 
        push $ skillTestModifier attrs iid (EnemyEvadeActionCriteria $ CriteriaOverride $ EnemyCriteria (ThisEnemy $ NonEliteEnemy <> EnemyAt ConnectedLocation))
