...
instance Show (SomeListField a) where
  show (SomeListField f) = show f

instance Eq (SomeListField a) where
  SomeListField (f1 :: Field a typ1) == SomeListField (f2 :: Field a typ2) = case eqT @typ1 @typ2 of
    Just Refl -> f1 == f2
    Nothing -> False

instance ToJSON (SomeListField a) where
  toJSON (SomeListField f) = toJSON f
