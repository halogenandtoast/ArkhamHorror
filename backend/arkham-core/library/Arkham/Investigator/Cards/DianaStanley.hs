...
import Arkham.Matcher hiding (PlaceUnderneath, AssetCard, EventCard)
...
instance HasAbilities DianaStanley where
  getAbilities (DianaStanley a) =
    ...
    where
      fewerThan5CardBeneath = if length (investigatorCardsUnderneath a) < 5 then NoRestriction else Never
      ...
      unless (null cardsUnderneath) $ do
        pushAll $
          ...
          , chooseOrRunOne iid
            ...
            : [ TargetLabel (CardIdTarget $ toCardId c) [AddToHand iid c] | c <- cardsUnderneath]
          , UnfocusCards]
      pure i
    UseCardAbility iid (isSource attrs -> True) 1 [Window _ (Window.CancelledOrIgnoredCardOrGameEffect source)] _ -> do
      let
        getSource = \case
          AssetSource aid -> field AssetCard aid
          SkillSource sid -> field SkillCard sid
          EventSource eid -> field EventCard eid
          AbilitySource abilitySource _ -> getSource abilitySource
          _ -> error $ "Unhandled source for Diana Stanley: " <> show source
      card <- getSource source
      ...
      pushAll
        $ [RemovedFromPlay source | canLeavePlay]
