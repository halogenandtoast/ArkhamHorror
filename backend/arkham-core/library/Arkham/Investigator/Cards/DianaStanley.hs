...
instance HasAbilities DianaStanley where
  getAbilities (DianaStanley a) =
    [ limitedAbility (PlayerLimit PerPhase 1)
        ...
        $ ReactionAbility
            (CancelledOrIgnoredCardOrGameEffect $ SourceOwnedBy You <> NotSource
              (SourceIsType InvestigatorType)
            )
            Free
    ]
   where
    fewerThan5CardBeneath = if length (investigatorCardsUnderneath a) < 5
      then NoRestriction
      else Never
...
instance RunMessage DianaStanley where
  runMessage msg i@(DianaStanley attrs) = case msg of
    ResolveToken _drawnToken ElderSign iid | iid == toId attrs -> do
      ...
      pure i
    UseCardAbility iid (isSource attrs -> True) 1 [Window _ (Window.CancelledOrIgnoredCardOrGameEffect source)] _
      -> do
        let
          getSource = \case
