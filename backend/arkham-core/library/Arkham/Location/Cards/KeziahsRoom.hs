...
instance HasAbilities KeziahsRoom where
  getAbilities (KeziahsRoom (a `With` _)) = withBaseAbilities a [mkAbility a 1 $ ReactionAbility (SkillTestResult Timing.When You (WhileInvestigating $ LocationWithId $ toId a) (SuccessResult AnyValue)) Free]
...
instance RunMessage KeziahsRoom where
  runMessage msg l@(KeziahsRoom (attrs `With` meta)) = case msg of
    ...
    DrewFromScenarioDeck iid _ target [card] | isTarget attrs target -> do
      ...
      pushAll [placement, chooseOne iid [Label "Do not move" [], Label "Move to location" [MoveTo $ move (toSource attrs) (toTarget iid) lid]]]
      pure l
