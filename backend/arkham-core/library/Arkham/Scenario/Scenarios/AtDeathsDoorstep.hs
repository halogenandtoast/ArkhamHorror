...
instance HasTokenValue AtDeathsDoorstep where
  getTokenValue iid tokenFace (AtDeathsDoorstep attrs) = case tokenFace of
    Skull -> do
      ...
      pure . uncurry (toTokenValue attrs Skull) $ if isHaunted then (1, 2) else (3, 4)
  ...
  { campaignLogRecordedSets = mapFromList
    [ ( MissingPersons
      , [ crossedOut (toCardCode i)
        | i <- [Investigators.gavriellaMizrah, Investigators.jeromeDavids, Investigators.valentinoRivas, Investigators.pennyWhite]
        ]
      )
    ]
  }
...
instance RunMessage AtDeathsDoorstep where
  runMessage msg s@(AtDeathsDoorstep attrs) = case msg of
    ...
    PreScenarioSetup -> do
      ...
      pushAll [ story iids introPart1, story iids introPart2, story iids introPart3, story iids introPart4 ]
      pure s
    StandaloneSetup -> do
    ...
    Setup -> do
      ...
      acts <- genCards
        [ Acts.hiddenAgendas
        , Acts.theSpectralRealm
        , Acts.escapeTheCage
        ]

      (entryHallId, placeEntryHall) <- placeLocationCard Locations.entryHallAtDeathsDoorstep
      (officeId, placeOffice) <- placeLocationCard Locations.office
      (billiardsRoomId, placeBilliardsRoom) <- placeLocationCard Locations.billiardsRoom
      (balconyId, placeBalcony) <- placeLocationCard Locations.balconyAtDeathsDoorstep
      otherPlacements <- traverse placeLocationCard_
      ...
      let
        ...
        removeClues = map (\n -> chooseOrRunN lead n [targetLabel l [RemoveClues (toTarget l) 1] | l <- [entryHallId, officeId, billiardsRoomId, balconyId]]) (splitBy4 noTimes)
      ...
      pushAll
        $ [ SetEncounterDeck encounterDeck
          ...
          <> otherPlacements
          <> [PlaceClues (toTarget entryHallId) 6 | toCardCode Investigators.gavriellaMizrah `elem` missingPersons ]
          <> [PlaceClues (toTarget officeId) 6 | toCardCode Investigators.jeromeDavids `elem` missingPersons ]
          <> [PlaceClues (toTarget billiardsRoomId) 6 | toCardCode Investigators.valentinoRivas `elem` missingPersons ]
          <> [PlaceClues (toTarget balconyId) 6 | toCardCode Investigators.pennyWhite `elem` missingPersons ]
          <> removeClues
    ...
    ResolveToken _ ElderThing iid -> do
      ...
      when isSpectralEnemy $
        push $ InvestigatorAssignDamage iid (TokenEffectSource ElderThing) DamageAny 1 (if isHardExpert attrs then 1 else 0)
      pure s
