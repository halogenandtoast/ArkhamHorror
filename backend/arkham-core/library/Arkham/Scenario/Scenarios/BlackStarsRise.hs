...
instance RunMessage BlackStarsRise where
  runMessage msg s@(BlackStarsRise attrs) = case msg of
    Setup -> do
      ...
      ashleighInterviewed <-
        elem (Recorded $ toCardCode Assets.ashleighClarke)
      ...
      let tokenToAdd = case scenarioDifficulty attrs of
                         Easy -> MinusThree
                         Standard -> MinusFive
                         Hard -> MinusSix
                         Expert -> MinusSeven
      ...
      pushAll $
        ...
        <> [AddToken tokenToAdd, PlaceLocation porteDeLAvancee, MoveAllTo (toSource attrs) (toLocationId porteDeLAvancee)]
