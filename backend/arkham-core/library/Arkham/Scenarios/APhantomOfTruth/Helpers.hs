...
getTheOrganist :: (HasGame m, Store m Card) => m EnemyId
getTheOrganist = selectJust $ EnemyWithTitle "The Organist"

investigatorsNearestToTheOrganist
  :: (HasGame m, Store m Card) => m (Distance, [InvestigatorId])
...
investigatorsNearestToEnemy
  :: (HasGame m, Store m Card) => EnemyId -> m (Distance, [InvestigatorId])
...
moveOrganistAwayFromNearestInvestigator :: (HasGame m, Store m Card) => m Message
...
disengageEachEnemyAndMoveToConnectingLocation
  :: (HasGame m, Store m Card) => m [Message]
