...
class HasTokenValue1 env f where
  getTokenValue1 :: (MonadReader env m, MonadIO m) => f p -> InvestigatorId -> TokenFace -> m TokenValue
...
class HasTokenValue env a where
  getTokenValue :: (MonadReader env m, MonadIO m) => a -> InvestigatorId -> TokenFace -> m TokenValue
  default getTokenValue :: (Generic a, HasTokenValue1 env (Rep a), MonadReader env m, MonadIO m) => a -> InvestigatorId -> TokenFace -> m TokenValue
