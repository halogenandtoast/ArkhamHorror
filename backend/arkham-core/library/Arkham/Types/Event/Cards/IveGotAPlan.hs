...
iveGotAPlan iid uuid =
  IveGotAPlan $ baseAttrs iid uuid "02107"
...
instance (HasCount ClueCount env InvestigatorId) => HasModifiersFor env IveGotAPlan where
  getModifiersFor (SkillTestSource iid _ _ (Just Fight)) (InvestigatorTarget _) (IveGotAPlan attrs) = do
    clueCount <- unClueCount <$> getCount iid
    pure $ modifiers attrs [ DamageDealt (min clueCount 3) ]
