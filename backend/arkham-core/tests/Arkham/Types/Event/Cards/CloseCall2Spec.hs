...
spec = describe "Close Call (2)" $ do
  it "shuffles the enemy just evaded back into the encounter deck" $ do
    ...
    enemy <- testEnemy id
    game <-
      runGameTest investigator [addToHand investigator (PlayerCard closeCall2), EnemyEvaded (getId () investigator) (getId () enemy)] (enemies %~ insertEntity enemy)
      >>= runGameTestOptionMatching "Play card" (\case
            Run{} -> True
            _ -> False)
