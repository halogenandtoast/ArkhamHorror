...
type ServerResult =
  ...
  | { tag: 'GameAudioSequence'; contents: string[] }
...
function playAudioFile(fileName: string) {
  ...
  // Accept either a simple root filename (legacy cues) or a safe relative path
  // (card cues with folder hierarchy and numbered variants).
  if (!/^[a-zA-Z0-9_.-]+\.(ogg|mp3|wav)$/i.test(path) && !isSafeAudioPath(path)) return
...
function audioUrl(path: string): string {
  ...
  const audioHost = siteSettings.audioHost.replace(/\/$/, '')
  return audioHost ? `${audioHost}/audio/${encodedPath}` : `/audio/${encodedPath}`
}

// Only allow relative paths (letters/digits/spaces/common title punctuation,
...
// frontend/scripts/generate-audio-manifest.cjs from whatever files are on
// disk, so the number of variants is never hardcoded here.
let audioManifestPromise: Promise<Record<string, string[]>> | null = null
function loadAudioManifest(): Promise<Record<string, string[]>> {
  if (!audioManifestPromise) {
    audioManifestPromise = fetch(audioUrl('manifest.json'))
...
/**
 * Play a sound for the first cue key that has any matching file(s) in the
 * audio manifest, choosing a random variant among them each time (to avoid
 * always hearing the same sound). Falls through to the next key (e.g. a
 * trait-based generic fallback) if a variant fails to load, and silently
 * plays nothing if none of the keys match anything.
 */
function playAudioCandidates(keys: string[]) {
  if (soundsDisabled.value) return
  loadAudioManifest().then((manifest) => {
    const groups = keys.map((key) => (manifest[key] ?? []).filter(isSafeAudioPath)).filter((variants) => variants.length > 0)
    ...
    const tryNextGroup = () => {
      ...
      groupIndex += 1
      tryVariant(remaining)
    }
    const tryVariant = (remaining: string[]) => {
      if (remaining.length === 0) {
        tryNextGroup()
        return
      }
      const path = pickRandom(remaining)
      const audio = new Audio(audioUrl(path))
      audio.addEventListener(
        'error',
        () => tryVariant(remaining.filter((p) => p !== path)),
        { once: true },
      )
      audio.play().catch((error) => console.warn(`Unable to play audio file: ${path}`, error))
    }
    tryNextGroup()
  })
}
...
const handleResult = (result: ServerResult) => {
  ...
  switch (result.tag) {
    ...
    case 'GameAudio':
      ...
      return
    case 'GameAudioSequence':
      playAudioCandidates(result.contents)
      return
    case 'GameUI':
