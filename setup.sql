...
BEGIN
    ...
    IF EXISTS (
        ...
        -- Check if the step to be deleted is higher than the current step of the game
        IF OLD.step <= (
            SELECT step
            FROM arkham_games
            WHERE id = OLD.arkham_game_id
        ) THEN
            RAISE EXCEPTION 'Cannot delete step % because it is equal to or less than the current step in the game.', OLD.step;
        END IF;
    END IF;
...
--
-- Name: arkham_steps prevent_deletion_of_invalid_step; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER prevent_deletion_of_invalid_step BEFORE DELETE ON public.arkham_steps FOR EACH ROW EXECUTE FUNCTION public.check_step_before_delete();
